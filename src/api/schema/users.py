...
from pydantic import BaseModel, Field, EmailStr, SecretStr, validator, root_validator
...
class Users(BaseModel):
    ...
    __uuid__: UUID = Field(description="Unique IDentifier", default_factory=uuid4, alias='uuid')
    ...
    gender: Optional[str] = Field(choice(["Male", "Female", "Nonbinary"]), description="User gender identification")
    pronouns: Optional[str] = Field(choice(["He/His", "She/Her", "They/Them"]), description="Pronoun to use when saying users")

    # ? Private fields
    password: SecretStr = Field(token_urlsafe(16), description="User password", alias='password')
    salt: SecretStr = Field(token_urlsafe(128), description="Salt for password", alias='salt')
